...
1. Przeczytać definicje poszczególnych zmiennych (pomogą w tym pliki MS Excel jak i sam Bank Danych Lokalnych) oraz określić charakter zmiennych: stymulanty i destymulanty.
2. Wczytać dane z plików MS Excel lub csv (gdyby były problemy z wgraniem danych na colab):
...
3. Utworzyć nowe zbiory danych dla każdego wskaźnika:
...
4. Połączyć wszystkie zbiory w jeden, większy zbiór danych zawierający wszystkie wskażniki oraz następujące kolumny: `kod`, `kazwa`, `rok` wykorzystując `pd.merge()`.
...
6. Utworzyć dwa zbiory danych: `analiza2015` i `analiza2022`.
7. Dla każdego zbioru niezależnie utworzyć dwa wskaźniki:
    ...
    + aby utworzyć ranking mozna zbiór danych posortować i utworzyć kolumnę używając metody `rank('dense', ascending=False)`.
8. Utworzyć zbiór `wyniki` łączący `analiza2015` i  `analiza2022` zawierający zmienne:
  + `kod`, `kazwa`, oraz 
  + wskażniki z punkty 7
9. Porównać rankingi odpowiadające sobie między latami z wykorzystaniem:
  + wykresu rozrzutu
  + porównania pozycji w rankingu
  + wyznaczenia różnicy w rankingach
10. zidentyfikować różnice między rankingami
